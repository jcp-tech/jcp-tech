...
const updateActiveLink = (currentSection) => {
    ...
    const updateLink = (link) => {
        ...
        if (href === currentSection) {
            ...
            link.classList.remove('text-gray-600', 'dark:text-white/80', 'hover:text-gray-900', 'dark:hover:text-white');
        } else {
            link.classList.remove('text-primary', 'font-bold', 'shadow-glow-text');
            link.classList.add('text-gray-600', 'dark:text-white/80', 'hover:text-gray-900', 'dark:hover:text-white');
        }
    };
...
const updateRunButtonState = () => {
    ...
    if (isRunning) {
        btn.classList.remove('bg-green-500/10', 'text-green-600', 'dark:text-green-400', 'hover:bg-green-500/20', 'hover:shadow-[0_0_10px_rgba(74,222,128,0.3)]', 'cursor-pointer');
        btn.classList.add('bg-yellow-500/10', 'text-yellow-500', 'cursor-wait');
        icon.textContent = 'progress_activity';
        icon.classList.add('animate-spin');
        text.textContent = 'Running...';
    } else {
        btn.classList.add('bg-green-500/10', 'text-green-600', 'dark:text-green-400', 'hover:bg-green-500/20', 'hover:shadow-[0_0_10px_rgba(74,222,128,0.3)]', 'cursor-pointer');
...
const showTerminalOverlay = () => {
    ...
    content.innerHTML = '<div class="animate-pulse inline-block w-2 h-4 bg-green-400 ml-1 mt-1"></div>';
};
...
const startTyping = async () => {
    ...
    for (let i = 0; i < fullOutput.length; i++) {
        ...
        lineDiv.className = `min-h-[1.5em] ${
            fullOutput[i].includes('Program finished') || fullOutput[i].includes('Press ENTER') 
    ...
    cursor.className = 'animate-pulse inline-block w-2 h-4 bg-green-400 ml-1 mt-1';
...
const filterProjects = (category) => {
    ...
    buttons.forEach(btn => {
        if (btn.dataset.filter === category) {
            ...
            btn.classList.remove('bg-white', 'dark:bg-white/5', 'border-gray-200', 'dark:border-white/10', 'hover:bg-gray-50', 'dark:hover:bg-white/10', 'text-gray-600', 'dark:text-white/80');
        } else {
            btn.classList.remove('bg-primary/10', 'border-primary', 'text-primary', 'font-bold');
            btn.classList.add('bg-white', 'dark:bg-white/5', 'border-gray-200', 'dark:border-white/10', 'hover:bg-gray-50', 'dark:hover:bg-white/10', 'text-gray-600', 'dark:text-white/80');
        }
    });
...
const switchExperience = (expId) => {
    ...
    buttons.forEach(btn => {
        if (btn.dataset.id === expId) {
            btn.classList.add('bg-gradient-to-r', 'from-primary/10', 'to-transparent');
            ...
            textSpan.classList.remove('text-gray-500', 'dark:text-white/60', 'group-hover:text-gray-900', 'dark:group-hover:text-white');
            ...
            btn.classList.remove('bg-gradient-to-r', 'from-primary/10', 'to-transparent');
            ...
            textSpan.classList.add('text-gray-500', 'dark:text-white/60', 'group-hover:text-gray-900', 'dark:group-hover:text-white');
...
const switchSkillCategory = (category) => {
    ...
    skills.forEach(skill => {
        if (skill.dataset.skillCategory === category) {
            ...
            skill.classList.add('animate-in', 'fade-in', 'zoom-in-95', 'duration-300');
        } else {
